...
public class ReportsService {
    ...
    public List<ScenarioResultDTO> getReport() {
        List<ScenarioResultProjection> projections = reportsRepo.fetchScenarioReport();
        List<ScenarioResultDTO> result = new ArrayList<>();

        for (ScenarioResultProjection p : projections) {
            ScenarioResultDTO dto = new ScenarioResultDTO();
            dto.setBusinessScenario(p.getBusinessScenario());
            ...
            dto.setCombinedKey(p.getCombinedKey());
            dto.setTcActivity(p.getTcActivity());
            ...
            dto.setTcResponsible(p.getTcResponsible());
            dto.setScenario(p.getScenario());
            dto.setScreenShotjpg(p.getScreenShotjpg());
            dto.setStartDate(p.getStartDate());
            dto.setSuccessCriteria(p.getSuccessCriteria());
            dto.setTestData(p.getTestData());
            dto.setTestedBy(p.getTestedBy());
            dto.setTcTransactionKey(p.getTcTransactionKey());
            dto.setTcWorkStream(p.getTcWorkStream());
            result.add(dto);
        }
        ...
        public List<ScenarioResultDTO> getDetailedTransactions() {
            ...
            return results.stream().map(r -> {
                ...
                dto.setType(String.valueOf(r[i++]));
                dto.setAction(String.valueOf(r[i++]));
